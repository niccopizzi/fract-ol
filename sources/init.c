...
/*   Updated: 2024/11/04 19:37:05 by npizzi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	calculate_fractal(t_data data, t_fractal fractal)
{
	...
	mlx_put_image_to_window(data.mlx_ptr, data.window_ptr, data.image->image, 0,
		0);
}
...
void	initialize_data_struct(char type, double complex c)
{
	...
	image.image_address = mlx_get_data_addr(image.image, &image.bits_per_pixel,
			&image.line_len, &image.endianess);
	fractal.c = c;
	fractal.scale = 4.0;
	fractal.x_start = -2.0;
	fractal.y_start = -2.0;
	fractal.brightness = 1.0;
	fractal.type = type;
	fractal.max_iter = 25;
	data.image = &image;
...
int	main(int argc, char *argv[])
{
	...
	if (argc < 2)
		print_error_and_exit(NULL,
			"Ooops.. it seems you forgot to give me the fractal to render!\n");
